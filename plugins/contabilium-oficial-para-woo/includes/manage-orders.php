...
function contabilium_send_order( $order_id, $order ) {
	...
	$response = $api->postRequest( 
		'https://rest.contabilium.com/notificador/wordpress',
		$new_comprobante
	);
	...
	if ( $comprobante_id>0 ) {
		...
		$msg = 'Error al enviar orden a Contabilum el ' . date( 'd/m/Y' ) . ' .Solicitud: ' . json_encode( $new_comprobante );
	}
...
function contabilium_refund_order( $order_id, $order ) {
	...
	if ( $comprobante_id>0  ) {
		update_post_meta( $order_id, 'cb_comprobante_id', $comprobante_id );
		$msg = 'Orden #'.$comprobante_id .' actualizada en Contabilum el ' . date( 'd/m/Y' ). ' .Solicitud: ' . json_encode( $new_comprobante );
	} else {
		$msg = 'Error al enviar orden a Contabilum el ' . date( 'd/m/Y' ) . ' .Solicitud: ' . json_encode( $new_comprobante );
	}
