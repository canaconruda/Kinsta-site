...
class HubwooObjectProperties {
	...
	public static function hubwoo_ecomm_sync_deal( $order_id, $source, $customer_id ) {
		...
		$update_order_status_id = get_option( 'mwb_update_order_ID' );

		if ( ! empty( $update_order_status_id ) ) {
			...
			do {
				$updated_response = HubWooConnectionMananager::get_instance()->ecomm_sync_status( $update_order_status_id, $object_type );
				$attempts++;
		...
		update_option( 'mwb_upddate_order_ID', '' );
		...
		if ( count( $deal_updates ) ) {
			...
			if ( $flag ) {
				...
				if ( 204 == $response['status_code'] ) {
					...
					update_option( 'mwb_update_order_ID', $order_id );
