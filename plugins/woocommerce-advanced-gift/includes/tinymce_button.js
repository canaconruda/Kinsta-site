(function(){
	...
	tinymce.PluginManager.add('wc_gift_shortcodes_button', function( editor, url ) {
        editor.addButton( 'wc_gift_shortcodes_button', {
            text: '',
			title:'Advance Gift Rule Plugin',
            icon: 'fa-check',
            onclick: function() {
				var width = jQuery(window).width(), H = jQuery(window).height(), W = ( 720 < width ) ? 720 : width;
				W = W - 80;
				H = H - 84;
				tb_show( 'Woocommerce Gift Shortcode', '#TB_inline?width=' + W + '&height=' + H + '&inlineId=flash_sale-shortcodes-form' );
            }
        });
    });
...
#flash_sale-shortcodes-form-tab_accordion .flash_sale-shortcodes-toggle-icon-list { margin: 0 0 10px 0; }\
		...
		<div class="flash_sale-shortcodes-form-types">\
			<ul>\
				<li class="flash_sale-shortcodes-form-type-button active" type="rule_list"><a href="#">Rule Shortcode</a></li>\
			</ul>\
		</div><!-- end types -->\
		<div class="flash_sale-shortcodes-form-tabs">\
<!-- product_rule -->\
			<div class="flash_sale-shortcodes-form-tab" id="flash_sale-shortcodes-form-tab_rule_list" style="display:block">\
				<h2>Add List Rule</h2>\
				...
				<table cellpadding="0" cellspacing="0">\
					<tr>\
						...
						<td>\
							<select fieldname="rules" id="select_rule" class="select_rule" multiple="multiple">\
								<option value="">Select Rule</option>\
							</select>\
						</td>\
					</tr>\
					<tr>\
						<th><label>Header Backcolour</label></th>\
						<td>\
							<input type="text" class="flash_sale-shortcodes-button_colour_custom" fieldname="header_backcolor" value="" />\
						</td>\
					</tr>\
					<tr>\
						<th><label>Header Text Colour</label></th>\
						<td>\
							<input type="text" class="flash_sale-shortcodes-button_colour_custom" fieldname="header_text_color" value="" />\
						</td>\
					</tr>\
					<tr>\
						<th><label>Show Date</label></th>\
						<td>\
							<select fieldname="pw_date">\
					...
					<tr>\
						...
						<td>\
							<select fieldname="pw_countdown_style">\
								<option value="style-1">Style 1</option>\
								<option value="style-2">Style 2</option>\
								<option value="style-3">Style 2</option>\
							</select>\
						</td>\
					</tr>\
					<tr>\
						<th><label>Show Category Depends</label></th>\
						<td>\
							<select fieldname="pw_cat_dep">\
								<option value="1">Yes</option>\
								<option value="0">No</option>\
							</select>\
						</td>\
					...
					<tr>\
						<th><label>Show Number of Products</label></th>\
						<td>\
							<select fieldname="nb_products">\
								<option value="1">Yes</option>\
								<option value="0">No</option>\
							</select>\
						</td>\
					...
					<tr>\
						<th><label>Show Cart Amount</label></th>\
						<td>\
							<select fieldname="cart_amount">\
								<option value="1">Yes</option>\
								<option value="0">No</option>\
							</select>\
						</td>\
...
<!-- end tab -->\
		...
		jQuery.ajax ({
			type: "POST",
			url: ajaxurl,
			data:   "action=pw_fetch_rule_gift",
			success: function(data) {
				//confirm(data);
				jQuery("#select_rule").html(data);
				jQuery('.select_rule').chosen();				
			}
		});
		/* Change tab */
		//var flash_sale_shortcode_type = "list_rule";
		var flash_sale_shortcode_type = "rule_list";
		...
		form.find('#flash_sale_shortcodes-submit').click(function(){
			...
			if(flash_sale_shortcode_type == "product_rulea") {
				...
				flash_sale_shortcode_code = flash_sale_shortcode_code + '[pw_woo_gift_' + flash_sale_shortcode_type + ']';
