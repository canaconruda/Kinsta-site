...
class Importer extends WXRImporter {
	...
	protected function get_reader( $file ) {
		...
		if ( ! class_exists( 'XMLReader' ) ) {
			$this->logger->critical( __( 'The XMLReader class is missing! Please install the XMLReader PHP extension on your server', 'freshio' ) );
		...
		if ( ! $status ) {
			$this->logger->error( __( 'Could not open the XML file for parsing!', 'freshio' ) );
	...
	public function import( $file, $options = array() ) {
		...
		if ( is_wp_error( $result ) ) {
			$this->logger->error( __( 'Content import start error: ', 'freshio' ) . $result->get_error_message() );
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'pt-importer/time_for_one_ajax_call', 20 ) ) {
			...
			$this->logger->info( __( 'New AJAX call!', 'freshio' ) );
